...
p. Textile class that is used to parse help files can be installed by saving "classTextile.php":https://github.com/netcarver/textile to the rah_plugcompile's directory (to same directory as @rah_plugcompile.php@). If the compiler is loaded as Textpattern plugin, the Textile class included with Textpattern is used.

p. If no Textile class is provided, target Textpattern installation's plugin installer's restricted raw Textile support is used as a fallback.
...
p. Note that in CLI all provided paths should be absolute (except standard input).
