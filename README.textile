...
p. The language code (i.e. ==#@language== line), as found in the @fi-fi.textile@, is required in all but the default textpack file (that's one file). If language code is included in all files, no default language is set, and user will see raw language strings if site's language isn't provided.
...
p. Note that in CLI all provided paths should be absolute (except stdout).
